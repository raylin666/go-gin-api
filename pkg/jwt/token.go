...
type Token interface {
	GenerateSign(userId int64, expireDuration time.Duration) (tokenString string, err error)
	ParseSign(tokenString string) (*claims, error)
}
...
// 生成TOKEN签名
func (t *token) GenerateSign(userId int64, expireDuration time.Duration) (tokenString string, err error) {
	// The token content.
	// iss: （Issuer）签发者
...
// 解析TOKEN签名
func (t *token) ParseSign(tokenString string) (*claims, error) {
